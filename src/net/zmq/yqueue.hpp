...
public:
    ...
    inline ~yqueue_t()
    {
        ...
        free(sc);  
        // 在C语言中，对NULL指针进行free操作是安全的，因为它相当于对一个空地址进行操作，不会引发错误或异常。
        // 实际上，标准库函数free()会检查传入的指针是否为NULL，如果是，则什么也不做，直接返回。
    ...
    {
        return begin_chunk->values[begin_pos];  // 这里为什么没有判空条件??????????????????????????????
    }
    ...
    {
        return back_chunk->values[back_pos];  // 这里为什么没有判空条件??????????????????????????????
    }
    ...
    inline void unpush()
    {
        ...
        else
        {
            ...
            free(end_chunk->next);           // 这里为什么不保存为空闲块呢???????????????????????????
            end_chunk->next = NULL;
        }
    }

    //  Removes an element from the front end of the queue.
    inline void pop()   // 这里也没有判空条件???????????????????????????????
    { 
...
private:
    // Individual memory chunk to hold N elements. 保存N个元素的单独内存块
    // 链表结点称之为chunk_t
    struct chunk_t
    {
    ...
    int end_pos;   // 如果只有end_pos而没有back_pos时，当需要返回队列尾部元素时，
                   // 由于end_pos指向队列尾部元素下一个位置，如果end_pos=0,那么返回尾部元素就比较复杂，需要访问到前一个chunk的最后一个位置。

    //  People are likely to produce and consume at similar rates.  In
    //  this scenario holding onto the most recently freed chunk saves
    //  us from having to call malloc/free.
    atomic_ptr_t<chunk_t> spare_chunk; //空闲块（把所有元素都已经出队的块称为空闲块），读写线程的共享变量
    // 空闲块为什么要设置成原子指针: 因为可能同时被读写线程使用
